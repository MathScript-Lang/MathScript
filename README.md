...
make a package manager, MathGet(.py), btw comming back soon).

### MathScript(.cpp)

So, I went back from nothing, from the ground up. I, by the way, changed from Python to C++ as...
well... Python isn't really know for being efficient, nor that powerful. So, I went all in and...
...
with extra steps), and C# has a pretty high-level syntax and API, and runs fadt enough to not be
...
as arrays/lists to Cayley-Dickens hypercomplex algebras, actually good `_`-indexing, still sequences
...
sudden ideas) and integrated it into the CLI. Currently, as of Thurday the 5th of February 2026, the
...
- "not C!": Minecrat-style splash text, equivalent of "Not linear!"
